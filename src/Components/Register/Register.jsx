...
export default function Register() {
  ...
  let myvalidation = yup.object().shape({
    name: yup.string().min(3,"Minimum lenght is 10").max(10,"Max Length is 10").required("Name is required"),
    email: yup.string().email("not valid email").required("email is required"),
    password: yup.string().required("Password is required").min(6,"password min length is 6"),
    rePassword: yup.string().required("repassword is required").oneOf([yup.ref("password")],"not matching password"),
...
<h2 className='font-bold text-2xl text-center my-4 text-emerald-700'>
  ...
  <div className="relative z-0 w-full mb-5 group">
      ...
      <label htmlFor="name" className="peer-focus:font-medium left-0 absolute text-sm text-gray-500 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto peer-focus:text-blue-600  peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Enter Your Name</label>
...
<span className='font-medium'>{formik.errors.password}</span>
  ...
  <div className="relative z-0 w-full mb-5 group">
      ...
      type="password" 
      name="rePassword"
      value={formik.values.rePAsswrodpassword}
      onChange={formik.handleChange}
      onBlur={formik.handleBlur} 
      id="re-password" 
      className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none  focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required />
      <label htmlFor="re-password" className="peer-focus:font-medium left-0 absolute text-sm text-gray-500 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto peer-focus:text-blue-600  peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Enter Your Repassword</label>
